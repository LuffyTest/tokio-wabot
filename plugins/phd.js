...
case 'phdl':
                        ...
                        downloader.phDl(url)
                            .then(async ({ title, download_urls, thumbnail_url }) => {
                                ...
                                if (count !== 2) {
                                    ...
                                    await conn.sendFileFromUrl(from, thumbnail_url, `${title}`, `Title: ${title}\n\nLinks:\n${shortsLow} (240P)\n${shortsMid} (480P)\n${shortsHigh} (720P)`, id)
                    ...
                    if (limit.isLimit(sender.id, _limit, limitCount, isPremium, isOwner)) return await conn.reply(from, ind.limit(), id)
                    limit.addLimit(sender.id, _limit, isPremium, isOwner)
                    await conn.reply(from, ind.wait(), id)
                    try {
                        nsfw.phDl(url)
                            .then(async ({ title, download_urls, thumbnail_url }) => {
                                const count = Object.keys(download_urls).length
                        ...
                        console.error(err)
                        await conn.reply(from, 'Error!', id
                    }
